...
import {
  decodeMultipleMasksCPU,
  decodeMultiplePartMasksCPU,
} from "./decode_multiple_masks_cpu";
...
export function toPersonKSegmentation(
  ...
  return tf.tidy(
    () => tf.cast(tf.equal(segmentation, tf.scalar(k)), "int32") as tf.Tensor2D
  );
}

export function toPersonKPartSegmentation(
  segmentation: tf.Tensor2D,
  bodyParts: tf.Tensor2D,
  k: number
): tf.Tensor2D {
  return tf.tidy(() =>
    tf.sub(
...
export async function decodePersonInstanceMasks(
  ...
  if (isWebGlBackend()) {
    const personSegmentations = tf.tidy(() => {
      ...
      const masksTensor = tf
        .engine()
        .makeTensorFromDataId(
          masksTensorInfo.dataId,
          masksTensorInfo.shape,
          masksTensorInfo.dtype
        ) as tf.Tensor2D;

      return posesAboveScore.map((_, k) =>
        toPersonKSegmentation(masksTensor, k)
      );
    });

    personSegmentationsData = (await Promise.all(
      personSegmentations.map((mask) => mask.data())
    )) as Uint8Array[];
  ...
  return personSegmentationsData.map((data, i) => ({
    data,
...
export async function decodePersonInstancePartMasks(
  ...
  if (isWebGlBackend()) {
    ...
    partSegmentationsByPersonData = (await Promise.all(
      partSegmentations.map((x) => x.data())
    )) as Int32Array[];
  ...
  //@ts-ignore
  return partSegmentationsByPersonData.map((data, k) => ({
